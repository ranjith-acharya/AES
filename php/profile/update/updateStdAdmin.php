...
<header>
	<nav class="navbar bg-white" style="padding-top:14px;padding-bottom:14px;box-shadow:0px 0px 6px rgba(0,0,0,0.4);">
		<a class="navbar-brand">Update&nbsp;Information&nbsp;Student</a>
...
<form method="POST" action="updateStdAddressAdmin.php" class="form container">
	<div class="lead text-capitalize">Change Address</div><br>
	<input type="text" name="updateStdAddmission" id="stdAddmission" placeholder="Enter Student Addmission Number" class="col-md-8"><br><br>
	...
	<form method="POST" action="updateStdContactAdmin.php" class="form container">
	<div class="lead text-capitalize">Change Contact Number</div><br><br>
	<input type="text" name="updateStdAddmission" id="stdCAddmission" placeholder="Enter Student Addmission Number" class="col-md-8"><br><br>
...
<script>$(document).ready(function(){$("#btnAdd").click(function(){var addmission = $("#stdAddmission").val();var address = $("#stdAddress").val();if(addmission==""){$("#stdAddmission").css({"border-color":"#cd201f","transition":".5s"});$("#stdAddmission").focus();return false;}if(address==""){$("#stdAddress").css({"border-color":"#cd201f","transition":".5s"});$("#stdAddress").focus();return false;}});$("#btnCnt").click(function(){var addmission = $("#stdCAddmission").val();var contact = $("#stdContact").val();var number = /^([0-9]){1,}$/;if(addmission==""){$("#stdCAddmission").css({"border-color":"#cd201f","transition":".5s"});$("#stdCAddmission").focus();return false;}if(contact==""){$("#stdContact").css({"border-color":"#cd201f","transition":".5s"});$("#stdContact").focus();return false;}if(number.test(contact)==false){$("#stdContact").css({"border-color":"#cd201f","transition":".5s"});$("#stdContact").focus();return false;}if(contact.length>10 || contact.length<10){$("#stdContact").css({"border-color":"#cd201f","transition":".5s"});$("#stdContact").focus();return false;}});});</script></body></html>